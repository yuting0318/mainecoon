...
const ImageResult: React.FC<ImageResultProps> = ({
    ...
    return <>
        <div className="mt-4">
            <div className="w-96 h-96 border-4 ml-4 " key="{seriesInstanceUID}" onClick={OnClick} >
                {// 將SM和ANN的內容放在同一個div內
                        ...
                        <div>
                            {// ModalityAttribute 是 SM 則顯示圖片
                                _.isEqual(_.first(modalityAttribute), "SM") &&(
                                    <>
                                        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/MI_with_contraction_bands_very_high_mag.jpg/1920px-MI_with_contraction_bands_very_high_mag.jpg"
                                             className="card-img-top" height={"500px"} width={"500px"} />
                                        <h5 className="text-green-500 mt-4 font-bold text-2xl ml-3">{modalityAttribute}</h5>
                                        <p className="fs-8 ml-3">最大圖片數量：{maxNumberOfFrames}張</p>
                                        <p className="fs-8 ml-3">擁有放大倍率：{seriesLevel}層</p>
                                    </>
                                )
                            }
                            {// ModalityAttribute 是 ANN 則顯示一支筆
                                _.isEqual(_.first(modalityAttribute), "ANN") && (
                                    <>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-3pencil-fill ml-20" viewBox="0 0 16 16" height={"230px"} width={"230px"}>
                ...
                {// ModalityAttribute 不是 SM 或 ANN 則顯示問號
                    (!_.isEqual(_.first(modalityAttribute), "SM") && !_.isEqual(_.first(modalityAttribute), "ANN")) && (
                        <>
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-question-circle " viewBox="0 0 16 16" height={"100px"} width={"100px"}>
